...
def index():
    ...
    if request.method == 'POST':
        ...
        width = int(float(request.form.get('width', 0) or 0))  # 정수로 변환
        height = int(float(request.form.get('height', 0) or 0))  # 정수로 변환
        ...
        for i in range(1, 11):
            ...
            if size and quantity:
                size = int(float(size) if size else 0)  # 정수로 변환
                ...
                if size > 0 and quantity > 0:
                    ...
                    total_length = int(total_length)  # 총 길이를 정수로 변환
                    total_weight += weight
